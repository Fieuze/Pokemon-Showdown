...
var parse = exports.parse = function (message, room, user, connection, levelsDeep) {
	...
	if (commandHandler) {
		var context = {
			...
			can: function (permission, target, room, cmdName) {
				if (!user.can(permission, target, room)) {
					this.sendReply("/" + (cmdName || cmd) + " - Access denied.");
					return false;
				}
				return true;
			},
...
var createSubParser = exports.createSubParser = function (commands, defaultHandler) {
	return function (target, room, user, connection, cmd, message, parserChain) {
		...
		this.can = function (permission, target, room, cmdName) {
			return this._can(permission, target, room, newParserChain.concat(newCmd || cmdName).join(' '));
		};
